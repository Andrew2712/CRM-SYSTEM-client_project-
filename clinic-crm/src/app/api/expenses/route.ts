  ...
  try {
    ...
    const expense = await prisma.expense.create({
      data: {
        ...
        category: category as ExpenseCategory, amount: parseFloat(amount),
        expenseDate: new Date(expenseDate), paymentMode: paymentMode as PaymentMode,
        createdById: session.user.id,
      },
      include: { createdBy: { select: { id: true, name: true, role: true } } },
    });

    await auditExpense(session, req, "CREATE", expense.id, {
      title: expense.title, amount: expense.amount,
      new: { category, amount, expenseDate, paymentMode },
    });

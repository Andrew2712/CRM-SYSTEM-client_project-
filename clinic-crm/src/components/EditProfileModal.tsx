...
/**
 * src/components/EditProfileModal.tsx  (NEW FILE)
 * ─────────────────────────────────────────────────────────────────────────────
 * Reusable modal for editing Patient OR Staff profiles.
 * Pass type="patient" | type="staff" and the entity data.
 */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          {/* Common fields */}
          <Field label="Name" name="name" value={(form as PatientFields).name ?? ""} onChange={handleChange} />
          <Field label="Phone" name="phone" value={(form as PatientFields).phone ?? ""} onChange={handleChange} />
          {userRole === "ADMIN" && (
            <Field label="Email" name="email" value={(form as PatientFields).email ?? ""} onChange={handleChange} type="email" />
          )}

          {/* Patient-only fields */}
          {type === "patient" && (
            <>
              <Field label="Age" name="age" value={String((form as PatientFields).age ?? "")} onChange={handleChange} type="number" />
              ...
              <Field label="Address" name="address" value={(form as PatientFields).address ?? ""} onChange={handleChange} multiline />
              <Field label="Purpose of Visit" name="purposeOfVisit" value={(form as PatientFields).purposeOfVisit ?? ""} onChange={handleChange} />
              <Field label="Medical Conditions" name="medicalConditions" value={(form as PatientFields).medicalConditions ?? ""} onChange={handleChange} multiline />
    ...
    <div>
      ...
      {multiline ? (
        <textarea
          ...
          onChange={onChange}
          rows={2}
          className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 resize-none"
        />
      ) : (
        <input
          type={type}
